...
def chart_view(request, genre):
    ...
    if genre not in genre_code_map:
        return render(request, 'crawling_app/error.html', {'message': 'Invalid genre'})

    genre_code = genre_code_map[genre]
    ...
    chart_data = [{'rank': i + 1, 'title': titles[i], 'singer': singers[i],
                   'album': albums[i], 'album_image': album_images[i]} for i in range(RANK)]
    return render(request, 'crawling_app/chart.html', {'chart_data': chart_data})
